...
AddEventHandler('fuel:startFuelUpTick', function(pumpObject, ped, vehicle)
	...
	if pumpObject then
		...
		if PlayerData.job.name == 'police' or PlayerData.job.name == 'mechanic' or PlayerData.job.name == 'ambulance' or PlayerData.job.name == 'taxi' then 
			local JobName = PlayerData.job.name
			useTankCardMenu(currentCost, JobName)
		else 
...
--Melding als je tank bijna leeg is
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(25000)
		...
		if IsPedInAnyVehicle(ped) then
			...
			if FuelLevel <= 25.0 and vehicle ~= nil and IsPedInVehicle(ped, vehicle, false) and (VehClass ~= 13) then
				ESX.ShowHelpNotification("Let op! Je tank is bijna leeg!")
			end
		end
...
function useTankCardMenu(currentCost, JobName)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'tankcard',
	{
		title    = 'Wil je je tankpas gebruiken?',
		align    = 'top-left',
		elements = elements
	}, function(data, menu)
